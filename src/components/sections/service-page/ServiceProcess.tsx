    ...
    <section className="bg-white py-20 scroll-mt-12" id="process-timeline">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 relative">
          ...
          <div className="hidden lg:block absolute top-[36px] left-[8%] right-[8%] h-0.5 bg-primary-700 -z-0"></div>
          ...
          {steps.map((step, idx) => (
            ...
            >
              {/* Step Circle */}
              <div className="w-12 h-12 rounded-xl bg-accent-orange text-white group-hover:bg-accent-hover text-white flex items-center justify-center shadow-md transition-colors duration-300 mb-5 relative">
                {step.icon}
                <div className="absolute -top-1.5 -right-1.5 bg-primary-700 group-hover:bg-accent-orange text-white text-[9px] font-extrabold w-4.5 h-4.5 rounded-full flex items-center justify-center border-2 border-white transition-colors duration-300">
                  {step.id}
                </div>
              </div>
...
// Why Choose Aegis Cards (6 cards)
export function ServiceWhyChoose() {
  const cards = [
    { label: "Professional Team", icon: <Wrench className="w-5 h-5 text-accent-orange" />, desc: "Expert technicians trained in high-floor safety anchoring rules." },
    { label: "Premium Quality", icon: <ShieldCheck className="w-5 h-5 text-accent-orange" />, desc: "Copolymer UV-treated nylon nets and marine-grade SS316 steel wires." },
    { label: "Affordable Pricing", icon: <Award className="w-5 h-5 text-accent-orange" />, desc: "Get fair pricing and transparent bills without hidden extra markups." },
    { label: "Fast Installation", icon: <Clock className="w-5 h-5 text-accent-orange" />, desc: "Most residential balcony netting setups are completed in 2-4 hours." },
    { label: "Excellent Support", icon: <Heart className="w-5 h-5 text-accent-orange" />, desc: "Dedicated support team available pre- and post-installation." },
    { label: "Clean Finish", icon: <CheckSquare className="w-5 h-5 text-accent-orange" />, desc: "We vacuum dust and leave anchoring zones clean and aesthetic." },
  ];

  return (
    <section className="bg-[#F7F5F0] py-20 scroll-mt-12" id="why-choose-aegis">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {cards.map((card, idx) => (
            ...
            >
              <div className="p-3 bg-[#F7F5F0] rounded-xl group-hover:bg-orange-50 transition-colors">
                {card.icon}
              </div>
            ...
            <div className="flex flex-col gap-4">
              {related.slice(0, 5).map((service) => (
                <Link
                  ...
                  className="flex items-center justify-between p-4 bg-[#F7F5F0] hover:bg-orange-50 border border-slate-200/50 rounded-xl hover:border-accent-orange/20 transition-all group"
                >
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                {areas.map((area) => (
                  <span
                    key={area}
                    className="text-xs font-semibold px-3.5 py-2 bg-[#F7F5F0] border border-slate-200 rounded-lg text-slate-600 hover:text-accent-orange hover:border-accent-orange/30 cursor-default transition-all"
                  >
                    {area}
                  </span>
                ))}
...
export function ServiceFAQs({ faqs, serviceName }: FAQsProps) {
  ...
  return (
    <section className="bg-[#F7F5F0] py-20 scroll-mt-12" id="faqs-service">
